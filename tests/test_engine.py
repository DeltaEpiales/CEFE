...
def test_thermal_entropy_scaling():
    ...
    grid = ce.CausalDiamondGrid(radius=4.0, spacing=0.8)
    engine = ce.EntropicFieldEngine(grid)
    engine.set_mass(1.0)
    
    engine.evolve_to_slice(target_t=0.0, temperature=0.0)
    s_vacuum = engine.compute_entanglement_entropy(1.5)
    
    engine.evolve_to_slice(target_t=0.0, temperature=10.0)
    s_thermal = engine.compute_entanglement_entropy(1.5)
...
def test_ads_volume_scaling():
    ...
    # In a uniform coordinate grid, the proper distance diverges near the boundary.
    # The true causal bounds (L * atan(r/L)) cause the lightcone to collapse faster in coordinate space.
    # Therefore, the exact causal diamond contains FEWER spacetime coordinate degrees of freedom.
